  ...
  const bg =
    tone === "ok"
      ...
      : tone === "bad"
        ...
        : "var(--slate-lit)";
  return (
    <span aria-hidden className="inline-block h-2 w-2 shrink-0" style={{ background: bg }} />
  );
}
          ...
          <div className="relative w-full max-w-md border border-hairline bg-ink p-6">
            ...
            {discovered.length + fallback.length > 0 ? (
              ...
              <p className="mt-6 font-sans text-meta text-slate-lit">
                No wallet extension announced itself to this page. Install
                MetaMask, Blade or HashPack and reload, and it will appear here.
              </p>
            )}
            ...
            {stalled && !connectError && (
              <div className="mt-5 border-l-2 border-oxblood-edge pl-4">
                ...
                <p className="prose-measure mt-3 font-sans text-data text-slate-lit">
                  The request was accepted by the extension but nothing came
                  back, which means its background worker has stopped. Look for a
                  wallet popup hiding behind this window first. If there is none,
                  open the extension from the toolbar to wake it, or reload it
                  from your browser&rsquo;s extensions page, then try again.
                </p>
              </div>
            )}
            ...
            {connectError && (
              <div className="mt-5 border-l-2 border-oxblood-edge pl-4">
                ...
                {/failed to connect|not (been )?(found|detected)|no provider/i.test(
                  ...
                  <p className="prose-measure mt-3 font-sans text-data text-slate-lit">
                    The extension did not answer. Open it once to wake it and
                    check it is unlocked, then try again. If your browser has its
                    own built-in wallet, such as Brave, set that browser to
                    prefer extensions for Ethereum — otherwise it displaces the
                    extension you picked. Any other wallet listed above will work
                    in the meantime.
                  </p>
                )}
              </div>
            ...
            <div className="mt-6 flex items-start gap-3">
              ...
              <p className="font-sans text-data leading-relaxed text-slate-lit">
                This opens your real wallet and asks it for {FUND_CHAIN.name},
                chain {FUND_CHAIN.id}. Hedera settles as EVM, so any wallet that
                speaks the JSON-RPC relay works. Nothing is signed by connecting.
              </p>
            </div>
          </div>
        </div>
